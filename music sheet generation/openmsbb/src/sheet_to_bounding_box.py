...
def launch_threaded_searchers(img, index, original, nb_thread):
    thread_list = []
    ...
    for box in boxes:
        if box[0][1] >= 1000 or box[0][0] >= 1000 or box[1][1] >= 1000 or box[1][0] >= 1000 or box[1][1] - box[0][1] <=4 or box[0][1] == 96 or box[0][1] == 122:
            continue
        else:
            new_box.append(box)
    ...
    new_box2 = []
    for box1 in new_box:
        area = abs(box1[1][1] - box1[0][1]) * abs(box1[1][0] - box1[0][0])
        if area > 50:
            new_box2.append(box1)
        else:
            for box2 in new_box:
                if box1 != box2 and abs(box1[1][1] - box2[0][1]) <= 2:
                    #print("area 1:", abs(box1[1][1] - box1[0][1]) * abs(box1[1][0] - box1[0][0]))
                    # print(box2)
                    a = min(box1[0][0],box2[0][0])
                    b = min(box1[0][1],box2[0][1])
                    c = max(box1[1][0],box2[1][0])
                    d = max(box1[1][1],box2[1][1])
                    new_box2.append(([a,b],[c,d]))
                    # new_box.remove(box1)
                    # new_box.remove(box2)
                    # print("a:", a, "b:", b, "c:", c, "d:", d)
                    # break
                # elif abs(box1[1][1] - box2[0][1]) > 2 and box1 not in new_box2:
                #     new_box2.append(box1)

    new_box1 = []
    for box in new_box2:
        if abs(box[1][1] - box[0][1]) >= 20:
            # print(box)
            # print(box[0][0], box[0][1], box[1][0], box[1][1] - abs(box[1][1] - box[0][1])/2)
            # print(box[0][0], box[0][1] + abs(box[1][1] - box[0][1])/2, box[1][0], box[1][1])
            new_box1.append(([box[0][0], box[0][1]], [box[1][0], box[1][1] - abs(box[1][1] - box[0][1])//2]))
            new_box1.append(([box[0][0], box[0][1] + abs(box[1][1] - box[0][1])//2], [box[1][0], box[1][1]]))
        else:
            new_box1.append(box)

    # new_new_box = []
    # for box in new_box1:
    #     area = abs(box[1][1] - box[0][1]) * abs(box[1][0] - box[0][0])
    #     if area > 50:
    #         new_new_box.append(box)

    #new_box = new_box[1:-1]
    cv2.rectangle(original, (0, 0), (5, 5), (60, 60, 60), 1)
    for box in new_box1:
    ...
    filename = "dataset/boundingbox/sheet_" + index + ".csv"
    df = pd.read_csv(filename)
    for i in range(len(new_box1)):
        df.loc[i, "top_x"] = new_box1[i][1][1]
        df.loc[i, "top_y"] = new_box1[i][1][0]
        df.loc[i, "bottom_x"] = new_box1[i][0][1]
        df.loc[i, "bottom_y"] = new_box1[i][0][0]
...
def process_image(img, index, nb_threads=0):
    ...
    img = launch_threaded_searchers(img, index, original, int(nb_threads))
    cv2.imshow('final', image_resize(img, height=1080))
    filename = "dataset/boxpic/sheet_" + index + ".png"
...
def main(argv):
    ...
    if argv[0][-4:] == '.pdf':
        ...
        index = argv[0][18:-4]
        process_image(img, index, nb_thread)
    return 0
